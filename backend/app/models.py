...
class AuditLog(db.Model):
    """系统审计日志模型"""
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 操作用户
...
class ProjectMilestone(db.Model):
    ...
    status = db.Column(db.String(20), default='pending')  # 状态(pending/completed/delayed)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)  # 创建时间
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)  # 更新时间
...
class ProjectMember(db.Model):
    """项目成员模型，用于管理项目成员关系"""
    id = db.Column(db.Integer, primary_key=True)
    project_id = db.Column(db.Integer, db.ForeignKey('project.id'), nullable=False)  # 关联项目ID
...
class ProjectDocument(db.Model):
    ...
    uploaded_by = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # 上传人
